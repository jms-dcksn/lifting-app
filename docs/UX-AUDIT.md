...
- [x] No `loading.tsx` skeletons — server navigations (home → session, history) show a
      dead white screen for the round-trip (P7 navigation item)
- [x] Header nav links have no active state
- [x] Fixed bottom bars (session finish, builder save) pair a `pb-28` magic spacer with
...
- [x] Slot cards show no sets-done vs target progress; prescription `3 × 5–8 @ 2 RIR` is
      static text; current slot doesn't read as *current*, completed slots don't recede
- [x] Logged-set rows appear/disappear instantly (no entry/exit animation); a failed
      optimistic write reverts silently
- [x] `swap` is still a caption-sized text link and wraps awkwardly under long names
      ("Shoulder Press (Life Fitness)") — promote to a real affordance (P7, differentiator)
- [x] Target line says "Start:" even after sets are logged this session (recommendation
...
- [x] Pattern filter is a text link, not a visible chip (P7 item)
- [x] Recent exercises sort first but aren't visually grouped (P7 item)
...
- [x] Day/slot reorder is an instant jump — P7 wants a physical move animation
...
- [x] Block status is a plain text line — P7 wants progress (week x of y) readable at a
      glance, CTA already carries the most visual weight
- [x] Start → session navigation has no pending skeleton (covered by global loading item)

## History

- [x] Chart hidden below 2 sessions with no explanation — needs the P7 empty state
- [x] Chart still default Recharts styling — restyle to the monochrome system (P7)
