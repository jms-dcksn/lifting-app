...
The Track Body page (`/analytics/body`) opens with measured weights (dots), trailing seven-calendar-day
...
1. Open Track → Explore → Body on a phone; check chart, goal line, and range buttons fit.
...
6. Change/remove the goal in You and revisit Track → Body; check distance/Set goal.
...
the current Settings goal. Range controls and 12-week bars stay on the Track Body weight card.
