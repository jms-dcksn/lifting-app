...
export function volumePageHref(exerciseId: string | null): string {
  if (!exerciseId) return "/analytics/volume";
  return `/analytics/volume?exercise=${encodeURIComponent(exerciseId)}`;
}
  ...
  return (
    <div className="flex min-w-0 flex-col gap-3">
      <div className="flex flex-wrap gap-2">
        <Chip href={volumePageHref(null)} selected={selectedId === null}>
          All training
        </Chip>
        {selected && (
          <Chip href={volumePageHref(selected.exerciseId)} selected>
            {selected.name}
          </Chip>
        )}
      </div>
      ...
      <ul className="divide-y divide-border">
        {filtered.map((item) => (
          <li key={item.exerciseId}>
            <Link
              href={volumePageHref(item.exerciseId)}
...
function Chip({
  ...
  return (
    <Link
      ...
      className={
        "inline-flex min-h-11 items-center rounded-full border px-3 py-1 text-caption font-medium " +
        (selected
